...
import pino from 'pino';
import QRCode from 'qrcode';
import { setupCronJobs } from './components/cronJobs.js';
import { handleMessages } from './components/messageHandler.js';
import { handleGroupParticipantsUpdate } from './components/groupManagement.js';
import { initDatabase } from './components/db.js';
import { safeSendMessage } from './components/utils.js';
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      console.log('Connecté à WhatsApp!');
      // Ensure sock.user is defined before accessing botJid
      if (!sock.user || !sock.user.id) {
        console.error('sock.user is undefined. Waiting for user data...');
        return;
      }
      const botJid = sock.user.id.replace(/:\d+/, '');
      
      // Setup cron jobs only after botJid is available
      await setupCronJobs(sock, botJid);

      // Handle messages
      sock.ev.on('messages.upsert', async ({ messages, type }) => {
        if (type !== 'notify') return;
        await handleMessages(sock, messages[0], botJid);
      });

      // Handle group participant updates
      sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
        await handleGroupParticipantsUpdate(sock, id, participants, action);
      });

      // Send initial message to creator
      await safeSendMessage(sock, CREATOR_JID, { text: 'Mon créateur, je suis en ligne 🙂‍↔️🥺🥹🥺' }, 500);

      // Periodic status message
      setInterval(async () => {
        try {
          await safeSendMessage(sock, CREATOR_JID, { text: '🤖 Connexion établie, le bot est opérationnel !' }, 500);
        } catch (err) {
          console.error('Erreur message périodique:', err.message);
        }
      }, 600000);
